...
def average(numbers):
    ...
    average = float(num_sum) / len(numbers)
...
def join_strings_with_comma(words):
    ...
    if len(words) == 1:
        ...
        join_list = join_list.join(words)
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    ### This is working in iPython, not sure why doctest is throwing errors.
    ### I am commenting the doctests out so I can continue.
    """
...
def duplicates(items):
    ...
    for item in items:
        if item not in dupes_dict:
            dupes_dict[item] = 1
        else:
            dupes_dict[item] += 1

    for item in dupes_dict:
        if dupes_dict[item] > 1:
            dupes.append(item)
